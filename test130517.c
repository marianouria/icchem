...
void allocmemory()
{
        ...
        if (xx == NULL || rr == NULL || erg == NULL || set == NULL) {
...
void readdata()
{
        ...
        double alpha, beta, gamma, sac, sb;
        ...
        while (fgets(c, sizeof(c), stdin) != NULL) {
                ...
                if (sscanf(c, "Segments: number alpha beta gamma dac db %*d")
                    == EOF) {
                        i = 1;
                        twist_eq = 0.;
                        arc_eq = 0.;
                ...
                if (sscanf(c, "%d %lf %lf %lf %lf %lf",
                           &numbr, &alpha, &beta, &gamma, &sac, &sb) == 6) {
        ...
        printf("Segments = %d   Arc = %g   Tw = %g   Lk = %d   SqDiam = %g\n",
               NS, arc_eq / (2 * M_PI), twist_eq / (2 * M_PI), Lk, diam2);
}
